def mru(pageReferenceDescription, frameCount):
  pageReferences = pageReferenceDescription.split("-")
  pageReferences[:] = [x[:-1] for x in pageReferences if x]
  frames = []
  pageHitCount = 0
  for pageReference in pageReferences:
    # Page is present in memory
    if pageReference in frames:
      pageHitCount += 1
      frames.remove(pageReference)
      frames.append(pageReference)
...
# minFrameCount = 5
# maxFrameCount = 10
# print(mru(pageReferenceDescription, 4))
...
#for i in range(minFrameCount, maxFrameCount):
 # print(mru(pageReferenceDescription, i))