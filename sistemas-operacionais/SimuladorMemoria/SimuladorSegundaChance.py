...
def segunda_chance(Re,memoriaFisica = 4,memoriaFisicaMax=5):
  listFrames=[]
  listAcertos= []
  loop2=memoriaFisicaMax-memoriaFisica+1
  memoriaFrame=memoriaFisica

  acertos = 0

  Re = Re.replace('R',"")
  Re = Re.replace('W',"")    #tratar entrada
  Re = Re.replace('-'," ")
  if(Re[-1]==''):
    del Re[-1]
  #print(len(Re)) 
  
  loop = memoriaFisicaMax-memoriaFisica+1
  ...
  def printHitsAndFaults(reference_string, frames):
    ...
    for i in range(l):
      ...
      if not findAndUpdate(x,arr,second_chance,frames):
        ...
        # Update page faults
        pf += 1
    listAcertos.append(abs(1000-pf))
    #print("Total de acertos", abs(1000-pf))
  ...
  for i in range(loop2):
    printHitsAndFaults(reference_string,memoriaFrame+i)
