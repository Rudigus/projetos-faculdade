def nur(pageReferenceDescription, frameCount, resetInterval):
  pageReferences = pageReferenceDescription.split("-")
  pageReferences[:] = [x for x in pageReferences if x]
  frames = []
  pageHitCount = 0
  counter = 0
  for pageReference in pageReferences:
    if counter % resetInterval == 0 and counter != 0:
    ...
    isInMemory = False
    for frame in frames:
      if pageReference[:-1] == frame[0]:
        isInMemory = True
        pageHitCount += 1
        frame[1] = 1
        frame[2] = max(frame[2], int(pageReference[-1] == 'W'))
    if isInMemory:
      continue
    # Page is not present in memory, but there are unoccupied frames
    elif len(frames) < frameCount:
      frames.append([pageReference[:-1], 1, int(pageReference[-1] == 'W')])
    # Page is not present in memory and all frames are occupied
    else:
      hasSwapped = False
      for i in range(4):
        for j in range(len(frames)):
          if frames[j][1] == i // 2 and frames[j][2] == i % 2:
            frames.pop(j)
            frames.append([pageReference[:-1], 1, int(pageReference[-1] == 'W')])
            hasSwapped = True
            break
